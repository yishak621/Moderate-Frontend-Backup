...
export default function DeleteBasicSiteSettingsModal({
  BasicSetting,
}: {
  ...
  const {
    deleteSiteSetting,
    deleteSiteSettingAsync,
    data,
    isDeleteSiteSettingError,
    isDeleteSiteSettingLoading,
    isDeleteSiteSettingSuccess,
    deletingSiteSettingError,
  } = useAdminDeleteSiteSetting(BasicSetting.key);
  const { close } = useModal();

  useEffect(() => {
    if (isDeleteSiteSettingSuccess) {
      toast.success("Setting deleted successfully!");
      close();
    }
  }, [isDeleteSiteSettingSuccess, close]);
    ...
    <div className=" bg-[#FDFDFD] min-w-[551px] p-10 rounded-[27px] flex flex-col">
      ...
      <div className="flex flex-row justify-between">
        <div className=" flex flex-col gap-1.5">
          <p className=" text-xl text-[#0c0c0c] font-medium">Delete Setting</p>
          <p className=" text-base font-normal text-[#717171] max-w-[303px]">
            Are you sure to delete <b>{BasicSetting?.key}</b> from your
            platform?
          </p>
        </div>
      ...
      <div className=" flex justify-center gap-3 items-center w-full mt-12">
        ...
        ${isDeleteSiteSettingLoading && "opacity-70 cursor-not-allowed"}`}
            disabled={isDeleteSiteSettingLoading}
            onClick={() => {
              deleteSiteSettingAsync();
            }}
          >
