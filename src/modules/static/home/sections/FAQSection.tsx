    ...
    <section className="py-20 w-full px-4">
      <div className="flex flex-col items-center mb-[100px]">
        <div className="flex flex-row items-center bg-[#f3f3f3] rounded-[41.5px] gap-2.5 py-3.5 px-6">
          ...
          <p className="text-[#000] text-base font-medium">FAQ</p>
        </div>
        <h3 className="mt-6 text-[32px] text-[#000] font-medium leading-normal">
      ...
      <div className="flex flex-col items-start gap-4 max-w-3xl mx-auto">
        {faqs.map((faq, index) => (
          <div
            key={faq.id}
            className="w-full bg-[#F7F7F7] rounded-[20px]  overflow-hidden transition-all duration-300 ease-in-out"
          >
            {/* Question Button */}
            <button
              onClick={() => toggleFAQ(index)}
              className="w-full flex items-center justify-between py-[37px] px-[49px] text-left transition-colors duration-200 hover:bg-[#EFEFEF]"
            >
              <p className="text-[#000] text-lg font-medium pr-8">
                {faq.question}
              </p>
              <div
            ...
            >
              <div className="px-[49px] pb-6 text-[#838383] text-base font-normal leading-relaxed">
                {faq.answer}
              </div>
            </div>
          </div>
